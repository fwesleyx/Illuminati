...
-- UPI_DIE_PREP items to determine optimal sourcing strategies based on manufacturing
...
-- These are the UPI_DIE_PREP items that will be analyzed
-- =====================================================================================
...
-- =====================================================================================
-- ITEM TYPE REFERENCE:
-- UPI_FAB : 0994          - Fabrication items
-- UPI_SORT : 0995         - Sort/test items  
-- UPI_ASSEMBLY: 0996      - Assembly items
-- UPI_TEST: 0997          - Test items
-- UPI_DIE_PREP: 1258      - Die preparation items (our focus)
-- UPI_BUMP: 1432          - Bump/packaging items
-- UPI_STACKED_SI: 1560    - Stacked silicon items
-- =====================================================================================

-- =====================================================================================
-- SECTION 2: DATA LOADING - IDENTIFY STARTING ITEMS
-- Load all UPI_DIE_PREP items that meet our criteria as starting points
-- These are filtered to ensure we only process relevant, active items
-- =====================================================================================

-- STEP1: DATA LOAD
-- PUSH all starting Item codes into #starting_stage table
-- These are the Leaf Nodes from where recursion will start
-- Filter and pick most optimal set of items to avoid recursion and cycles wasted on Post Validation

-- FILTER: only die prep items which have sort as their direct child
...
WHERE it.item_typ_dsc IN ('UPI_DIE_PREP')              -- Focus on die prep items only
...
WITH bom_hier_cte AS (
    ...
    -- Initialize the recursion with our starting UPI_DIE_PREP items
    -- =====================================================================================
    SELECT 
    ...
    WHERE bh.bom_level < 10                                                 -- Prevent infinite recursion
		...
		AND cit.item_typ_dsc IN ('UPI_DIE_PREP','UPI_SORT','UPI_BUMP','UPI_FAB')
)
...
-- =====================================================================================
-- SECTION 4: BOM VALIDATION AND FILTERING
-- Apply business rules to filter out invalid BOM structures
-- =====================================================================================

-- VALIDATE BOM 
-- FOR ALL THE ITEMS WHICH HAVE CHILD AS UPI_DIE_PREP DELETE THOSE WHICH DONT UPI_SORT AS THEIR CHILD
-- Business Rule: UPI_DIE_PREP items must have UPI_SORT as their immediate child
-- This ensures we only process valid manufacturing flows

DELETE #bom_temp
WHERE master_parent_cde IN (
	SELECT master_parent_cde 
	FROM #bom_temp 
	WHERE bom_level = 2                     -- Check immediate children
	AND item_type <> 'UPI_SORT'             -- Must be SORT type
)

-- =====================================================================================
-- SECTION 5: EXPAND BOM TO INCLUDE PARENT STAGES
-- Add the parent items of our UPI_DIE_PREP items to get complete manufacturing flow
-- =====================================================================================

-- PUSH NEXT STAGE INTO BOM TO GET THEIR SITE INDICATOR
-- PUSH THE NEXT STAGE OF UPI_DIE_PREP INSIDE BOM

-- Clean up temporary table if it exists
IF OBJECT_ID('tempdb..#item_child_parent_codes') IS NOT NULL
    DROP TABLE #item_child_parent_codes

-- Create table to store parent-child relationships for UPI_DIE_PREP items
CREATE TABLE #item_child_parent_codes(
	master_parent_cde VARCHAR(30),      -- Original master parent
	item_cde VARCHAR(30),               -- UPI_DIE_PREP item code
	item_rev VARCHAR(2)                 -- Item revision
	);

-- GET PARENT CODES FOR UPI_DIE_PREP ITEMS IN BOM
-- Extract all UPI_DIE_PREP items that need their parents added
INSERT INTO #item_child_parent_codes
SELECT master_parent_cde,item_cde,item_rev
FROM #bom_temp
WHERE item_type = 'UPI_DIE_PREP';

-- Add parent items of UPI_DIE_PREP items to complete the manufacturing flow
...
AND item_type = 'UPI_DIE_PREP';         -- Only update the die prep items
...
-- item_type: Manufacturing stage type (UPI_DIE_PREP, UPI_SORT, etc.)
