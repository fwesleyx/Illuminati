...
(
  ...
  assy_item_status VARCHAR(30), 
  ch_upi_test VARCHAR(40) DEFAULT NULL,
  ch_upi_assembly VARCHAR(100) DEFAULT NULL
...
INSERT INTO #ItemHierarchy (
	fg_item_cde, fg_item_rev, fg_item_dsc, fg_item_typ_cde, fg_item_sap_mat_typ, fg_item_typ_dsc,fg_item_status,
	test_item_cde, test_item_rev, test_item_dsc, test_item_typ_cde, test_item_sap_mat_typ, test_item_typ_dsc,test_item_status,
	assy_item_cde, assy_item_rev, assy_item_dsc, assy_item_typ_cde, assy_item_sap_mat_typ, assy_item_typ_dsc,assy_item_status
)
SELECT 
	...
	, ilvl0.nme				as fg_item_status

	, b.child_item_cde		as test_item_cde
	, b.parent_item_rev		as test_item_rev
...
FROM
	...
	JOIN [dbo].item_revision ir0 ON p.item_cde = ir0.item_cde AND p.mfg_rev = ir0.item_rev
	JOIN [speed_2max].[dbo].item_rls_lvl ilvl0 ON ir0.lvl_idn = ilvl0.lvl_idn 
	JOIN [dbo].bus_unit_dsgn_grp budg ON budg.bus_unit_idn = ir0.bus_unit_idn

	JOIN dbo.item c ON b.child_item_cde = c.item_cde
	JOIN dbo.item_type cit ON c.item_typ_cde = cit.item_typ_cde
	JOIN [dbo].item_revision ir1 ON c.item_cde = ir1.item_cde AND c.mfg_rev = ir1.item_rev
    ...
    JOIN [dbo].item_revision ir2 ON c1.item_cde = ir2.item_cde AND c1.mfg_rev = ir2.item_rev
	JOIN [speed_2max].[dbo].item_rls_lvl ilvl2 ON ir2.lvl_idn = ilvl2.lvl_idn 

WHERE
	p.sap_mat_typ = 'FERT' AND
    c1it.item_typ_dsc IN ('UPI_TEST','UPI_ASSEMBLY') 
      AND budg.short_nme IN ('IA','CIG')
    AND ilvl0.nme NOT IN ('INACTIVE','DRAFT','OBSOLETE')  
    AND ilvl0.nme = 'PRODN_APPROVED'
	 AND ilvl1.nme NOT IN ('INACTIVE','DRAFT','OBSOLETE')  
    AND ilvl1.nme = 'PRODN_APPROVED'
	 AND ilvl2.nme NOT IN ('INACTIVE','DRAFT','OBSOLETE')  
    AND ilvl2.nme = 'PRODN_APPROVED'


    update tgt 
    set tgt.ch_upi_assembly= src.val_txt
    ...
    update tgt 
    set tgt.ch_upi_test= src.val_txt
    from #ItemHierarchy tgt 
    join [speed_2max].[dbo].uda_item src on src.item_cde= tgt.test_item_cde
    join [speed_2max].[dbo].uda_definition def on def.att_idn = src.att_idn and def.att_nme='SUBASSY_PROD_ENGNR_CD'

    DELETE from #ItemHierarchy  
    where  ch_upi_test != 'PRODUCTION' or ch_upi_assembly!= 'PRODUCTION';
     --select top 100 * from #ItemHierarchy
	-- select *  from #ItemHierarchy; 

WITH bom_hier_cte AS (
		select 
	  p.fg_item_cde		as parent_item_cde
	, NULL		as child_item_cde
	, p.fg_item_rev		as parent_item_rev
	, p.fg_item_dsc		as fg_item_dsc
	, p.fg_item_typ_cde		as fg_item_typ_cde
	, p.fg_item_sap_mat_typ			as fg_item_sap_mat_typ
	, p.fg_item_typ_dsc		as fg_item_typ_dsc	
	, p.fg_item_status as fg_item_status
	, CAST(1 AS INT) as bom_level
	from
	#ItemHierarchy p
 union all
 	select 
	  p.test_item_cde		as parent_item_cde
	, p.test_item_cde		as child_item_cde
	, p.test_item_rev		as parent_item_rev
	, p.test_item_dsc		as fg_item_dsc
	, p.test_item_typ_cde		as fg_item_typ_cde
	, p.test_item_sap_mat_typ			as fg_item_sap_mat_typ
	, p.test_item_typ_dsc		as fg_item_typ_dsc	
	, p.test_item_status as fg_item_status
	, CAST(2 AS INT) as bom_level
	from
	#ItemHierarchy p
	union all
 	select 
	  p.assy_item_cde		as parent_item_cde
	 , p.assy_item_cde		as child_item_cde
	, p.assy_item_rev		as parent_item_rev
	, p.assy_item_dsc		as fg_item_dsc
	, p.assy_item_typ_cde		as fg_item_typ_cde
	, p.assy_item_sap_mat_typ			as fg_item_sap_mat_typ
	, p.assy_item_typ_dsc		as fg_item_typ_dsc	
	, p.assy_item_status as fg_item_status,
	CAST(3 AS INT) as bom_level
	from
	#ItemHierarchy p
	
	)
	--select * from bom_hier_cte
...
SELECT 
    parent_item_cde as master_parent_cde,
    child_item_cde as item_cde,
    parent_item_rev as item_rev, 
    fg_item_status as item_status,
    fg_item_typ_dsc as item_type,
    fg_item_dsc as item_dsc,
    bom_level as bom_level,
...
WHERE item_cde IN (SELECT item_cde FROM #item_site WHERE ext_site_ind = 'N') AND item_cde NOT IN (SELECT item_cde FROM #item_site WHERE ext_site_ind = 'Y')

-- TO BE SRICTLY EXTERNAL ALL NEED IN EXTERNAL AND NONE IN INTERNAL
UPDATE #bom_temp
SET site_indicator = 'E'
WHERE item_cde IN (SELECT item_cde FROM #item_site WHERE ext_site_ind = 'Y') AND item_cde NOT IN (SELECT item_cde FROM #item_site WHERE ext_site_ind = 'N')

-- TO BE MIXED NEED ATLEAST ONE INTERNAL AND ONE INTERNAL

UPDATE bt
SET site_indicator = 'M', -- Mixed
    remark = 'MIXED SITE'
FROM #bom_temp bt
WHERE EXISTS (SELECT 1 FROM #item_site WHERE item_cde = bt.item_cde AND ext_site_ind = 'Y')
  AND EXISTS (SELECT 1 FROM #item_site WHERE item_cde = bt.item_cde AND ext_site_ind = 'N')
